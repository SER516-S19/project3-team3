...
public class QuizEditView extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					String title = "Which mountain is located in Washington?";
					ArrayList<String> choices = new ArrayList<String>(
							Arrays.asList("Mt. Rainer", "Mt. Saint Helens", "Mt. Rushmore", "Peter Jackson"));
					String correctChoice = "Mt. Rainer";
					QuestionModel exampleQuestion = new QuestionModel(title, choices, correctChoice);

					ArrayList<QuestionModel> questions = new ArrayList<QuestionModel>();
					questions.add(exampleQuestion);

					QuizModel exampleQuiz = new QuizModel("my hardcoded quiz title", questions);
		...
		saveButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int questionComponent = 2; questionComponent < numberOfComponents - 1; questionComponent++) {
					...
					QuestionAndChoicesPanel currentQuestionPanel = (QuestionAndChoicesPanel) parentPanel
							.getComponent(questionComponent);
